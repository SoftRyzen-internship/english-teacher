...
export const Modal = ({ children, isOpen, setIsOpen, contentClassName, buttonClassName, childContentClassName}: ModalProps) => {
  return (
    <Dialog.Root open={isOpen} onOpenChange={setIsOpen}>
      <Dialog.Portal> 
        <Dialog.Overlay className="fixed inset-0 bg-semiTransparentGray"/>
          <Dialog.Content
          className={clsx('absolute transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 max-h-[90vh]  w-[89%] ',
             contentClassName)}
          ...
          >
            <button
             className={clsx('absolute text-black hover:text-buttonClickPink focus:text-buttonClickPink transition-all duration-300',
               buttonClassName)}
            ...
            <div className={clsx(childContentClassName)}>
              {children}
            </div>
        </Dialog.Content>
      </Dialog.Portal>
